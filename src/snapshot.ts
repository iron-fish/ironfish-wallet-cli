...
export class SnapshotDownloader {
  ...
  async download(
    onDownloadProgress: (prev: number, curr: number) => void,
  ): Promise<void> {
    const manifest = await this.manifest()
    ...
    const idleInterval = setInterval(() => {
      ...
      if (timeSinceLastChunk > idleTimeout) {
        ...
        idleCancelSource.cancel(
          `Download timed out after ${TimeUtils.renderSpan(
            timeSinceLastChunk,
          )}`,
        )
      }
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      .catch((error) => {
        if (idleCancelSource.token.reason?.message) {
          ...
          throw new Error(
            `Error while downloading snapshot file: ${ErrorUtils.renderError(
              error,
            )}`,
          )
        }
...
export class DownloadedSnapshot {
  ...
  async replaceDatabase(): Promise<void> {
    await fsAsync.rm(this.chainDatabasePath, {
      recursive: true,
      force: true,
      maxRetries: 10,
    })
