...
export class BridgeCommand extends IronfishCommand {
  ...
  async start(): Promise<void> {
    ...
    if (flags.watch) {
      ...
      this.log(
        `Run ironfishw transaction ${hash} to get the status of your transaction.`,
      )
      this.log(
        `Run ironfishw transaction:watch ${hash} to wait for the status of your transaction to be confirmed/ expired.`,
      )
    }
  }

  private async getAndValidateInputs(
    ...
    if (!asset) {
      const names = tokens.map(
        (t, index) =>
          `${index + 1}. ${t.name} (${t.symbol}) - ${t.web3_address}`,
      )
    ...
    const targetNetwork = await this.selectNetwork(
      networkId,
      targetNetworks,
      asset,
    )
  ...
  private displayTransactionSummary(
    ...
    const bridgeAmount = CurrencyUtils.render(
      BigInt(txn.bridge_output.amount) -
 ...
 Expiration                     ${
   raw.expiration ? raw.expiration.toString() : ''
 }
 `
