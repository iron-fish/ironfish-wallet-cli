...
export class Send extends IronfishCommand {
  ...
  async start(): Promise<void> {
    ...
    const memo =
      flags.memo ?? (await ui.inputPrompt('Enter the memo (or leave blank)'))
  ...
  private async sendTransactionWithLedger(
    ...
    const publicKey = (
      await client.wallet.getAccountPublicKey({ account: from })
    ).content.publicKey
    ...
    const unsignedTransaction =
      buildTransactionResponse.content.unsignedTransaction
