...
export class MultisigCreateDealer extends IronfishCommand {
  ...
  static flags = {
    ...
    importCoordinator: Flags.boolean({
      ...
      default: true,
      description:
        'Import the coordinator as a view-only account after creating key packages',
    }),
  }
  ...
  async start(): Promise<void> {
    ...
    for (const [
      i,
      { identity, account },
    ] of response.content.participantAccounts.entries()) {
      this.log('\n')
    ...
    this.log('Next step:')
    this.log(
      'Send the account imports to the participant with the corresponding identity.',
    )
  }
  ...
  async getCoordinatorName(
    ...
    const secretNames = new Set(
      identitiesResponse.content.identities.map((i) => i.name),
    )
