...
Balance is your coins from all of your transactions, even if they are on forks or not yet included as part of a mined block.`
  ...
  async start(): Promise<void> {
    ...
    const renderedAvailable = renderValue(
      response.content.available,
      asset,
      assetName,
    )
    const renderedConfirmed = renderValue(
      response.content.confirmed,
      asset,
      assetName,
    )
    const renderedUnconfirmed = renderValue(
      response.content.unconfirmed,
      asset,
      assetName,
    )
    const renderedPending = renderValue(
  ...
  explainBalance(
    ...
    const renderedUnconfirmedDelta = renderValue(
      unconfirmedDelta,
      asset,
      assetName,
    )
...
function renderValue(
  ...
  if (renderNameManually) {
    return `${assetName} ${CurrencyUtils.render(
      amount,
      false,
      asset.id,
      asset.verification,
