...
export class TransactionsImportCommand extends IronfishCommand {
  ...
  static hiddenAliases = ['transaction:add', 'transaction:import']
  ...
  async start(): Promise<void> {
    ...
    if (txArg) {
      ...
      transaction = await longPrompt(
        'Paste the hex-encoded transaction to import',
        {
          required: true,
        },
      )
    ...
    this.log(
      `Transaction imported for accounts: ${response.content.accounts.join(
        ', ',
      )}`,
    )
  }
