...
export class TransactionsDecodeCommand extends IronfishCommand {
  ...
  static flags = {
    ...
    account: Flags.string({
      char: 'f',
      description:
        'Name of the account to use to for viewing transaction details',
    }),
  ...
  async start(): Promise<void> {
    ...
    if (rawTransaction) {
      return await renderRawTransactionDetails(
        client,
        rawTransaction,
        account,
        this.logger,
    ...
    const unsignedTransaction =
      this.tryDeserializeUnsignedTransaction(transactionString)
    ...
    if (transaction) {
      return await renderTransactionDetails(
        client,
        transaction,
        account,
        this.logger,
  ...
  tryDeserializeRawTransaction(
    ...
    try {
      ...
      this.logger.debug(
        `Failed to deserialize transaction as RawTransaction: ${ErrorUtils.renderError(
          e,
        )}`,
      )
  ...
  tryDeserializeUnsignedTransaction(
    transaction: string,
  ...
  tryDeserializeTransaction(transaction: string): Transaction | undefined {
    try {
      ...
      this.logger.debug(
        `Failed to deserialize transaction as Transaction: ${ErrorUtils.renderError(
          e,
        )}`,
      )
