...
export const fetchChainportNetworkMap = async (
  ...
  return (
    await makeChainportRequest<{
      cp_network_ids: { [key: string]: ChainportNetwork }
    }>(url)
  ).cp_network_ids
}
...
export const fetchChainportVerifiedTokens = async (
  ...
  return (
    await makeChainportRequest<{ verified_tokens: ChainportVerifiedToken[] }>(
      url,
    )
  ).verified_tokens
}
...
export const fetchChainportBridgeTransaction = async (
  ...
  const url = `${
    config.endpoint
  }/ironfish/metadata?raw_amount=${amount.toString()}&asset_id=${
    asset.web3_address
...
const makeChainportRequest = async <T extends object>(
  url: string,
): Promise<T> => {
