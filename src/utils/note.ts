...
async function getNoteTreeSize(client: RpcClient) {
  ...
  const currentBlockSequence = parseInt(
    getCurrentBlock.content.currentBlockIdentifier.index,
  )
  ...
  return (
    getBlockResponse.content.block.noteSize -
    (config.content.confirmations || 2)
  )
}
