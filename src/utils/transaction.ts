...
import { BurnDescription } from '@ironfish/sdk/build/src/primitives/burnDescription'
import { MintDescription } from '@ironfish/sdk/build/src/primitives/mintDescription'
import { ux } from '@oclif/core'
import { RpcClientWallet } from '../command'
...
export class TransactionTimer {
  ...
  constructor(spendPostTime: number, raw: RawTransaction) {
    ...
    this.estimateInMs =
      spendPostTime > 0
        ? Math.max(Math.ceil(spendPostTime * raw.spends.length), 1000)
        : -1
  }
  ...
  start() {
    ...
    this.progressBar.start(100, 0, {
      estimate: TimeUtils.renderSpan(this.estimateInMs, {
...
export async function renderRawTransactionDetails(
  ...
  const assetLookup = await getAssetVerificationByIds(
    client,
    assetIds,
    account,
    undefined,
  ...
  const from = rawTransaction.spends.length
    ? rawTransaction.spends[0].note.owner()
    : null
...
Fee             ${feeString}
Expiration      ${
    rawTransaction.expiration !== null
      ? rawTransaction.expiration.toString()
      : ''
  }`
...
function collectRawTransactionAssetIds(
  rawTransaction: RawTransaction,
): string[] {
