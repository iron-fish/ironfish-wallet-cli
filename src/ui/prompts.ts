...
export async function multisigSecretPrompt(
  client: Pick<RpcClient, 'wallet'>,
): Promise<string> {
...
> {
  ...
  if (filter) {
    balances = balances.filter((balance) =>
      filter(assetLookup[balance.assetId]),
    )
  }
  ...
  balances = balances.sort((asset1, asset2) => {
    const verified1 =
      assetLookup[asset1.assetId].verification.status === 'verified'
    const verified2 =
      assetLookup[asset2.assetId].verification.status === 'verified'
  ...
  const choices = balances.map((balance) => {
    ...
    const assetNameWithVerification = renderAssetWithVerificationStatus(
      assetName,
      asset,
    )
